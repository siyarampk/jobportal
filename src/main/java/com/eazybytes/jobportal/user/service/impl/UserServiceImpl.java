...
public class UserServiceImpl implements IUserService {
    ...
    @Transactional
    @Override
    public void unsavedJob(String userEmail, Long jobId) {
        //Validate if user exists
        JobPortalUser user = userRepository.findJobPortalUserByEmail(userEmail)
                        .orElseThrow(() -> new RuntimeException("User not found with email " + userEmail));
    ...
    public JobApplicationDto applyForJob(String userEmail, ApplyJobRequestDto applyJobRequestDto) {
        ...
        return ApplicationUtility.mapToJobApplicationDto(savedApplication);
    }
    ...
    public List<JobApplicationDto> getJobSeekerApplications(String userEmail) {
        ...
        JobPortalUser user = userRepository.findJobPortalUserByEmail(userEmail)
                .orElseThrow(() -> new RuntimeException("User not found with email " + userEmail));
        return user.getJobApplications().stream().map(jobApp -> ApplicationUtility.mapToJobApplicationDto(jobApp))
                .collect(Collectors.toList());
    }
